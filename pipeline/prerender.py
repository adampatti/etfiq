...
<script type="application/ld+json">{json.dumps(ld, separators=(',', ':'))}</script><style>{STYLE}p.cite{{margin:22px 0 0;padding:12px 14px;background:#F0F3F7;border-left:3px solid #1D34F2;border-radius:0 8px 8px 0;font-size:13px;line-height:1.55;color:#2B2B2B}}p.cite .mono{{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;word-break:break-all}}table caption{{caption-side:top;text-align:left;font-size:12.5px;color:#6A6A68;padding:0 0 8px}}.doc dl dt{{font-weight:600;margin:18px 0 6px}}.doc dl dd{{margin:0;color:#2B2B2B}}.doc h2{{margin-top:28px}}.doc .lede{{font-size:17px;color:#2B2B2B}}</style></head>
...
def stats_page(funds, income, themes, core, as_b, as_i, as_t, extra):
    ...
    rows = ''.join(f'<tr><th>{esc(a)}<span class="sub">{esc(c)}</span></th><td class="fig">{esc(b)}</td></tr>' for a, b, c in lines)
    ...
    inner = (f'<p class="note">Recomputed every trading night. Figures as of {tdate(max(as_b, as_i, as_t))}.</p>'
             ...
             f'<table class="stat"><tbody>{rows}</tbody></table>'
...
def data_page(as_of, counts):
    ...
    cards = ''.join(f'<a href="/data/{fn}"><span class="lvl">{esc(lab)}</span><b class="file">{fn}</b><span>{esc(d)}</span></a>' for fn, lab, d in DATA_FILES)
    ld = [
    ...
    parts = [
        ...
        f'<h2>The files</h2><nav class="list">{cards}</nav>',
    ...
    return doc_page('data/', title, desc, ''.join(parts), ld, short='Open data')
...
def embed_block(desk, ticker, name):
    ...
    return (f'<h2>Put this on your own page</h2>'
            ...
            f'<p><img src="/embed/{desk}/{ticker}.svg" alt="{esc(ticker)} {EMBED_LABEL[desk]}, ETFIQ" width="640" loading="lazy" style="max-width:100%;height:auto;border-radius:3px"></p>'
...
STYLE = R.RAIL_CSS + R.FOOTER_CSS + """pre.embed{background:#0A0A0A;color:#EFEFED;padding:14px 16px;border-radius:3px;overflow-x:auto;font:12.5px/1.6 ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap;word-break:break-all}dl.faq{margin:0 0 8px}dl.faq dt{font-weight:600;margin:18px 0 6px}dl.faq dd{margin:0;color:#2B2B2B}nav.rel{display:flex;flex-wrap:wrap;gap:8px;margin:10px 0 6px}nav.rel a{border:1px solid #E6E6E3;border-radius:999px;padding:7px 13px;background:#fff;font-size:14px;text-decoration:none;color:#0A0A0A}nav.rel a:hover{border-color:#1D34F2}nav.rel.out a::after{content:' \\2197';color:#6A6A68}thead th .sub{font-weight:400;font-size:12px;color:#6A6A68;margin-top:4px;max-width:220px}table.stat th{width:auto;white-space:normal;font-weight:500;color:#0A0A0A;padding:12px 14px 12px 0}table.stat th .sub{display:block;font-weight:400;font-size:13.5px;line-height:1.4;color:#6A6A68;margin-top:3px}table.stat td.fig{width:1%;white-space:nowrap;text-align:right;vertical-align:top;padding:12px 0;font:600 20px/1.2 'Geist Mono',ui-monospace,SFMono-Regular,Menlo,monospace;font-variant-numeric:tabular-nums;color:#0A0A0A}body{margin:0;background:#FAFAF9;color:#0A0A0A;font:16px/1.55 Geist,system-ui,-apple-system,'Segoe UI',sans-serif}main{max-width:820px;margin:0 auto;padding:28px 20px 60px}h1{font-family:'Source Serif 4',Charter,'Iowan Old Style',Georgia,serif;font-weight:600;font-size:31px;letter-spacing:-.008em;line-height:1.15;margin:18px 0 8px}nav.list a b{font-family:'Source Serif 4',Charter,'Iowan Old Style',Georgia,serif}.lede{color:#2B2B2B;font-size:17.5px;line-height:1.5}.tk{font-family:'Geist Mono',ui-monospace,monospace;font-weight:600}.cta{display:inline-block;margin:14px 0 22px;padding:10px 16px;background:#1D34F2;color:#fff;border-radius:3px;text-decoration:none;font-weight:600}table{border-collapse:collapse;width:100%;margin:12px 0 18px;font-size:14.5px}th,td{text-align:left;padding:8px 10px;border-bottom:1px solid #E6E6E3;vertical-align:top}th{color:#2B2B2B;font-weight:500}table.kv tbody th{width:42%}table.cmp tbody th{width:26%;white-space:normal}table.cmp thead th{width:37%}main.doc.wide{max-width:1120px}nav.list{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(320px,100%),1fr));gap:12px;margin:14px 0 20px}nav.list a{display:block;padding:16px 18px;border:1px solid #E6E6E3;border-radius:3px;background:#fff;text-decoration:none;color:#0A0A0A}nav.list a:hover{border-color:#1D34F2}nav.list a .lvl{display:block;font:600 11px/1 Geist,system-ui,sans-serif;letter-spacing:.08em;text-transform:uppercase;color:#1D34F2;margin-bottom:8px}nav.list a b.file{font-family:'Geist Mono',ui-monospace,SFMono-Regular,Menlo,monospace;font-size:16px;letter-spacing:-.01em}nav.list a b{display:block;font-size:18px;line-height:1.28;letter-spacing:-.01em;margin-bottom:6px}nav.list a span{display:block;color:#2B2B2B;font-size:14.5px;line-height:1.45}table.pairs td{white-space:normal}table.pairs td:last-child{display:flex;flex-wrap:wrap;gap:5px}table.pairs td a{display:inline-block;padding:3px 8px;border:1px solid #E6E6E3;border-radius:999px;font-size:12px;white-space:nowrap;text-decoration:none;color:#0A0A0A;background:#fff}table.pairs td a:hover{border-color:#1D34F2}table.pairs td .more{color:#6A6A68;font-size:12px;align-self:center}.filterbox{display:flex;gap:8px;max-width:460px;margin:8px 0 16px}.filterbox input{flex:1;padding:10px 12px;border:1px solid #D2D2CE;border-radius:3px;font:14px Geist,system-ui,sans-serif}nav.crumb{font-size:13px;color:#6A6A68;margin:2px 0 14px;display:flex;flex-wrap:wrap;gap:6px;align-items:center}nav.crumb a{color:#0A0A0A;text-decoration:none}nav.crumb a:hover{text-decoration:underline}nav.crumb b{color:#D2D2CE;font-weight:400}nav.crumb span{color:#0A0A0A;font-weight:600}table.hub th:first-child,table.hub td:first-child{width:70px;white-space:nowrap}table.hub th:nth-child(3),table.hub td:nth-child(3){width:86px}table.hub td:nth-child(2){min-width:210px}table.hub td,table.hub th{white-space:normal}table.hub .tk{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-weight:600}.note{color:#6A6A68;font-size:13.5px}nav.desks a{margin-right:14px}main a{color:#0A0A0A}main p a,main li a,main td a,main dd a,main .lede a,main .note a{text-decoration:underline;text-decoration-color:#D2D2CE;text-decoration-thickness:1px;text-underline-offset:2px}main p a:hover,main li a:hover,main td a:hover,main dd a:hover,main .lede a:hover,main .note a:hover{text-decoration-color:#1D34F2}footer{margin-top:40px;color:#6A6A68;font-size:13px}"""
