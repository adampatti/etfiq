...
# ---------------------------------------------------------------- pages
STYLE = R.RAIL_CSS + R.FOOTER_CSS + """nav.rel{display:flex;flex-wrap:wrap;gap:8px;margin:10px 0 18px}nav.rel a{border:1px solid #E6E6E3;border-radius:999px;padding:6px 12px;background:#fff;font-size:13px;text-decoration:none;color:#0A0A0A}nav.rel a:hover{border-color:#1D34F2}body{margin:0;background:#FAFAF9;color:#0A0A0A;font:16px/1.6 Geist,system-ui,-apple-system,'Segoe UI',sans-serif}main{max-width:920px;margin:0 auto;padding:28px 20px 60px}.eyebrow{font:600 11px/1 Geist,sans-serif;letter-spacing:.08em;text-transform:uppercase;color:#6A6A68}h1{font-family:'Source Serif 4',Charter,'Iowan Old Style',Georgia,serif;font-weight:600;font-size:33px;letter-spacing:-.008em;margin:8px 0 8px;line-height:1.15}.dek{font-size:18.5px;line-height:1.5;color:#2B2B2B;margin:0 0 22px}.sum p{font-size:17.5px;line-height:1.55;margin:0 0 12px}.narr{border-left:2px solid #1D34F2;background:#fff;padding:14px 18px 16px;border-radius:0;margin:18px 0}.narr .lab{font:600 11px/1 Geist,sans-serif;letter-spacing:.06em;text-transform:uppercase;color:#6A6A68;margin-bottom:8px}h2{font-size:19px;margin:30px 0 10px;letter-spacing:-.01em}table{border-collapse:collapse;width:100%;font-size:14px;background:#fff;border:1px solid #E6E6E3;border-radius:3px}th,td{text-align:left;padding:8px 10px;border-bottom:1px solid #EFEFED;vertical-align:top}th{font:600 10.5px/1 Geist,sans-serif;letter-spacing:.06em;text-transform:uppercase;color:#6A6A68;background:#F4F4F2}td:not(:first-child),th:not(:first-child){text-align:right;font-variant-numeric:tabular-nums}.note{color:#6A6A68;font-size:13.5px;margin-top:24px}.cta{display:inline-block;margin:10px 0 0;padding:9px 14px;background:#1D34F2;color:#fff;border-radius:3px;text-decoration:none;font-weight:600}nav.list a{display:block;padding:14px 16px;border:1px solid #E6E6E3;border-radius:3px;background:#fff;margin:10px 0;color:#0A0A0A;text-decoration:none}nav.list b{display:block;font-size:18.5px;font-family:'Source Serif 4',Charter,'Iowan Old Style',Georgia,serif;font-weight:600;letter-spacing:-.008em}nav.list span{color:#6A6A68;font-size:14.5px;line-height:1.45}.wrap{overflow-x:auto}main a{color:#0A0A0A}main p a,main li a,main td a,main dd a,main .lede a,main .note a{text-decoration:underline;text-decoration-color:#D2D2CE;text-decoration-thickness:1px;text-underline-offset:2px}main p a:hover,main li a:hover,main td a:hover,main dd a:hover,main .lede a:hover,main .note a:hover{text-decoration-color:#1D34F2}footer{margin-top:40px;color:#6A6A68;font-size:13px}"""
...
def piece_page(pc, narrative, dated=None):
    url = f"{BASE}/research/{dated + '/' if dated else ''}{pc['slug']}.html"
...
def index_page(pieces, archive=None):
    ...
    arch = ''
    if archive:
        days = list(archive.items())[:60]
        arch = ('<h2>Archive</h2><p class="dek">Every set as it stood that night, frozen and permanently addressed.</p>'
                '<nav class="rel">' + ''.join(f'<a href="/research/{d}/">{fdate(d)}</a>' for d, _ in days) + '</nav>')
...
def archive_index(out):
    """Every dated set already written, newest first: {date: [(slug, title)]}."""
    days = {}
    for d in sorted((p for p in out.iterdir() if p.is_dir() and re.fullmatch(r'\d{4}-\d{2}-\d{2}', p.name)), key=lambda p: p.name, reverse=True):
        items = []
        for f in sorted(d.glob('*.html')):
            if f.name == 'index.html':
                continue
            m = re.search(r'<h1>(.*?)</h1>', f.read_text())
            items.append((f.stem, re.sub(r'<[^>]+>', '', m.group(1)) if m else f.stem))
        if items:
            days[d.name] = items
    return days


def day_page(day, items):
    url = f'{BASE}/research/{day}/'
    title = f'ETFIQ Research, {fdate(day)}'
    desc = f'What the ETFIQ desks showed on {fdate(day)}: one computed piece per desk, with the tables and the method.'
    links = ''.join(f'<a href="/research/{day}/{slug}.html"><b>{esc(t)}</b></a>' for slug, t in items)
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
<meta property="og:title" content="{esc(title)}"><meta property="og:description" content="{esc(desc)}"><meta property="og:url" content="{url}"><meta property="og:image" content="{BASE}/og.png">
<style>{STYLE}</style></head><body>{R.rail('research')}<main>
<div class="eyebrow">ETFIQ Research · archive</div><h1>{esc(title)}</h1>
...
def build():
    ...
    for pc in pieces:
        ...
        # a dated copy, so a finding keeps a permanent address someone can cite
        day = pc.get('asOf') or TODAY
        d = out / day
        d.mkdir(exist_ok=True)
        f = d / f"{pc['slug']}.html"
        if not f.exists():
            f.write_text(piece_page(pc, narrative, dated=day))
            archived += 1
    (out / 'index.html').write_text(index_page(pieces, archive_index(out)))
    for day, items in archive_index(out).items():
        (out / day / 'index.html').write_text(day_page(day, items))
    print(f'research: {len(pieces)} pieces, {archived} archived')
    for pc in pieces:
