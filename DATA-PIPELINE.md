...
## Themes desk (2026-09-05, phase one)

- **Universe and taxonomy**: `pipeline/thematic_universe.py` reads the SEC series and class file, keeps funds whose names carry a theme word and none of the exclusion words (income, buffer, leverage, bonds, spot crypto, mutual fund share classes), and assigns one primary theme per fund from an ordered list of twenty-seven. `THEME_OVERRIDES` and `FORCE` carry the funds the rules get wrong or miss. The taxonomy is editorial and Standards says so.
- **Holdings**: `pipeline/holdings.py` finds each fund's latest public N-PORT filing through EDGAR full-text search by SEC series id (or exact fund name when the SEC file has no id), parses positions with name, CUSIP, ISIN, ticker and percent of net assets, and drops cash and derivatives. The S&P 500 and Nasdaq-100 books come from the IVV and QQQM filings. Public N-PORT shows the last month of a quarter about sixty days later, so every card carries the filing date.
- **Overlap engine**: securities match on CUSIP, then ISIN, then ticker, then a normalised name. Weight overlap is the sum of the smaller weight of each matched security; active share is one minus that; in-index weight is the fund's weight in securities the index holds at all. `pipeline/thematic.py` computes each fund against both indexes, every fund against every other fund (a percent matrix carried in the page for the look-through tool), and the five closest peers.
- **Performance**: Tiingo prices through the income desk's functions, with the listing-gap trim, over the same windows, against SPY and QQQ, plus drawdown from the fund's all-time high. Funds with no trade in twelve days are treated as closed and left off the desk (kept in the snapshot).
- **Not yet**: expense ratios (needed for the active expense ratio), theme purity per holding, daily issuer holdings files, closures and flows on the theme map.
...
VistaShares (2026-09-05): the fund pages give the ten largest holdings, net assets and the fee. AIS and POW have full N-PORT books, which the pipeline prefers for overlap; AMMO, GALX and RTOO launched in July 2026 and show the issuer's top ten, assets and fee until their first N-PORT filing (public around late November), when overlap and active share appear. Partial books never feed the overlap engine. The themes universe also carries 47 funds the SEC series file lacks, found by sweeping Tiingo's listing names for theme words.
...
`pipeline/research.py` computes one piece per desk from the desks' files (tables, a computed summary, the method and the data file) and writes static pages under `site/research/`, rebuilt nightly and listed in the sitemap and llms.txt. `pipeline/draft.py` asks Claude for a narrative from those tables when `ANTHROPIC_API_KEY` is set, then checks the draft mechanically: every number must appear in the tables or the summary, the site's banned words are refused, no recommendation language, no em dashes, 150 to 700 words. Only a draft that passes is published, under the label ETFIQ Narrative (Adam's wording, 2026-09-05: no model credit on the page); it is redrafted when the tables change. Without the key the pages carry the computed summary alone. The drafter tries the model ids in `ETFIQ_DRAFT_MODEL` (comma separated, default claude-sonnet-5 then older Sonnet ids) with a one-token request first and prints the API's own error text when a call is refused, so a billing or key problem shows up in the run log by name.

## Brand assets (2026-09-05)

The crest (direction E) is built by `pipeline/brand.py` from outlined glyphs: `site/crest.svg`, `site/favicon.svg` (no rings, drawn for 16 pixels), `site/logo.svg` and `site/logo-dark.svg` (horizontal lockup with the tagline "Independent ETF data, in plain words"), `site/og.png` and `site/apple-touch-icon.png`. The header of the app and the rail of every static page carry the crest, the wordmark and the tagline inline; the tagline hides under 760 pixels. Colours: coin #161C24, ink #0F1419, blue #2457E6 on light and #5A87E5 on dark.

## Static explainers, hubs and embeds (2026-09-05)

Three additions aimed at search and at AI assistants, all in `pipeline/prerender.py` unless noted.

- **Explainers and standards as real pages.** `site/standards/` and `site/learn/{buffer,income,themes}.html` are lifted out of the app at build time by `app_doc()`, which reads the template literals in `site/index.html`, so the static page and the app can never drift. The glossaries carry DefinedTermSet markup, which is what assistants quote from. Stub pages at the old WordPress paths (`/about/`, `/contact/`, `/blog/`, `/services/`, `/hello-world/`) canonicalise to the new pages so any link the domain earned before 2025 still lands somewhere.
- **Hub pages, 96 of them.** One per issuer with two or more funds, one per theme, one per buffer reset month, each with ItemList markup, plus item lists on the three desk indexes. These match query shapes the site had no page for, such as an issuer's whole range or every fund resetting in a given month. A daily `site/changed/` page carries the computed lines from `insights.json`.
- **Embeddable graphics.** `pipeline/embed.py` writes 817 standalone SVG cards, one per fund, with literal colours and no font or script dependency: the outcome band, the payout bar or the difference bar, the figures behind it, the data date and the etfiq.com credit. Every fund page shows the card and the two-line snippet another site pastes to use it. Regenerated nightly, so an embedded card stays current on someone else's page.

Accuracy: census stages `embeds` (817 cards) and `docs` (the static explainers, the hubs and the changed page) check that every figure on a card matches its desk record and that no hub lists a fund that is not on that desk. Both were verified by deliberately corrupting a card and a hub. `shows()` now matches figures on a digit boundary, because a loose substring check let 0.8% stand in for 8%.

## Head to head pages and question blocks (2026-09-05)

`pipeline/prerender.py` writes a static page for every pair of the funds people search for on each desk: the twenty best known buffer and income funds and the twenty largest thematic funds, which is 190 pairs a desk and 570 pages under `site/compare/<desk>/<A>-<B>.html`, named alphabetically so a pair has one page. Each carries the desk's own fields for both funds in one table, the plain-words line for each, the overlap between the two books on the themes desk, a question block, and links to the neighbouring comparisons. Every fund page now ends with the same kind of question block, three to five questions in the words people type, answered only with figures already on the page, marked up as FAQPage; fund and comparison pages also carry BreadcrumbList. The whole layer is in the sitemap and llms.txt.

Accuracy: the census stages `compare` (570 pages) and `faqs` check that every figure on those pages matches the desk data and that every claim in a question block agrees with it, so a page cannot say one fund is cheaper than another unless the fee data says so. Both run in the nightly job.

## Accuracy census (2026-09-05)

`pipeline/census.py` recomputes every published number from the raw sources with fresh code (it imports none of the pipeline's arithmetic) and lists every discrepancy in `data/census/<date>.md` and `.json`; it exits 1 when anything fails. Stages: income (338 funds: every window, cash, price, total return, benchmark, gap, payout frequency and rate, trailing cash, since-launch figures, from the Tiingo cache), buffer (245 funds: every identity from the published terms, the page's derived fields, days remaining against the calendar, the reference return against the reference ETF's own closes, then a live re-read of the Innovator table with its header row, the AllianzIM model and eight FT Vest pages), themes (289 funds: windows against SPY and QQQ, drawdown, top-ten weight, overlap and active share with a fresh matcher, active fee, matrix shape, peers), payouts and sources, books (629), research tables and home page lines, and a sample of static pages. First full run, 2026-09-05: zero discrepancies after three fixes it surfaced. Two were reference-asset mislabels in `snapshot.py` (BGLD's SPDR Gold Trust and the two FT Vest equal-weight funds' Invesco S&P 500 Equal Weight ETF had matched "spdr" and "s&p 500" before "gold" and "equal weight"; the key list is now ordered from specific to general), found because the fund's reference return did not match SPY's closes. The third was the payout-frequency bug below. Run it with `python3 pipeline/census.py` (all stages) or name the stages; `live` fetches the issuer pages.

## Income desk corrections (2026-09-05)

Two corrections after Adam questioned the VistaShares paid figures. First, a fund younger than the chosen window used to show a shorter window's figures without saying so (ACKY, four days short of a year, showed its six-month cash under the one-year setting). It now shows its whole record since launch with a "since" date under the figure. Second, the payout-frequency function paired the wrong ex-dates when a fund had three to six distributions, so nine young monthly funds were labelled weekly and their annualised payout rates were multiplied by 52 instead of 12 (TPRY showed 65 percent; it is 15). Fixed in `pipeline/income.py`; the cadence in `pipeline/payouts.py` was computed separately and was right.

## Portfolio desk (2026-09-05, phase one)

`pipeline/books.py` writes one small file per fund under `site/data/books/` with its filed holdings (issuer daily file first, then the latest N-PORT), for every income and themes desk fund and the core index funds people hold; bond, cash, commodity and crypto funds are asset classes without a look-through; a fund whose book is mostly T-bills and options is marked synthetic with its economic exposure. Refreshed on Saturdays. The desk (`#/portfolio`) fetches only the books for the tickers in a portfolio. Positions live in the link (`TICKER:weight`, `TICKER:150S` shares, `TICKER:$25000`), and named portfolios are kept on the device. Four views: the look-through and overlap with the weighted fee; the buffer positions blended; cash by month for twelve months from the payout calendar, carried forward at each fund's cadence beyond the 45-day horizon and marked projected; and the outcome of a chosen S&P 500 move on each buffer position from its published cap and buffer, before fees, at period end, beside the unbuffered move. Compare shows the first four positions' desk cards side by side. Buffer funds enter the look-through as their reference index; synthetic income funds as their exposure; both labelled. No allocation is suggested anywhere.

Filing check (2026-09-05): the income universe now carries each fund's SEC series id, and `holdings.fund_holdings` accepts an N-PORT only when the filing's own series id matches (or, for a fund the SEC file does not list, its series name matches after registered marks are stripped). Before this, a name search could return a fund-of-funds filing that merely held the fund; JEPI's book was one such. Funds younger than their first N-PORT have no book yet and the desk says so.
